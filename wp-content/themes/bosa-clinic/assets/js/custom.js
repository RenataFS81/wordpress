...
*/
function bosa_clinic_slickNavHeight (){
...
*/
function bosa_clinic_wpMenuAccordion( selector ){
...
function bosa_clinic_fixed_header(){
  $notificationHight = $( '.notification-bar' ).height();
  $logo_selector = document.getElementById( 'headerLogo' );
  var width = $( window ).width();

  if ( $logo_selector && BOSACLINIC.fixed_nav && BOSACLINIC.fixed_header_logo ) { 
    if ( $mastheadHeight < $(window).scrollTop()){
      if ( BOSACLINIC.separate_logo == '' ){
        if ( BOSACLINIC.the_custom_logo !== '' ){
          $logo_selector.src = BOSACLINIC.the_custom_logo;
        }
      }else{
        $( '.site-header .site-branding img' ).css( 'display' , 'block' );
        if( !BOSACLINIC.mobile_fixed_nav_off || width >= 782 ){
          $logo_selector.src = BOSACLINIC.separate_logo;
        }
      }
    }else{
      if ( BOSACLINIC.header_two_logo !== '' && ( BOSACLINIC.is_front_page || BOSACLINIC.overlay_post || BOSACLINIC.overlay_page ) && BOSACLINIC.is_header_two ){
         $logo_selector.src = BOSACLINIC.header_two_logo;
      }else if ( BOSACLINIC.the_custom_logo !== '' ) {
          $logo_selector.src = BOSACLINIC.the_custom_logo;
      }else if ( BOSACLINIC.separate_logo !== '' ){
        $( '.site-header .site-branding img' ).css( 'display' , 'none' );
      }
    }
  }
  if ( $mastheadHeight > $( window ).scrollTop() || $( window ).scrollTop() == 0 ) {
      if ( BOSACLINIC.fixed_nav && $( '#masthead.site-header' ).hasClass( 'sticky-header' )){
          $( '#masthead.site-header' ).removeClass( 'sticky-header' );
          // Fixed header in admin bar
          if( BOSACLINIC.is_admin_bar_showing && width >= 782 ){
            $( '.fixed-header' ).css( 'marginTop', 0 );
          }
          if( BOSACLINIC.is_admin_bar_showing && width <= 781 ){
            $( '.fixed-header' ).css( 'marginTop', 0 );
          }
      }
  }else if ( BOSACLINIC.fixed_nav && !$( '#masthead.site-header' ).hasClass( 'sticky-header' )){
    if( !BOSACLINIC.mobile_fixed_nav_off || width >= 782 ){
      ...
      if( BOSACLINIC.is_admin_bar_showing && width <= 600 ){
        $( '.fixed-header' ).css( 'marginTop', 0 );
      }
      if( BOSACLINIC.mobile_fixed_nav_off && width <= 781 ){
        $( '.fixed-header' ).css( 'marginTop', 0 );
    }
  }
}
...
*/
function bosa_clinic_header_two_postion() {
  var width = $( window ).width();

  if ( BOSACLINIC.is_header_two ) {
    if( BOSACLINIC.is_admin_bar_showing && width >= 782 ){
...
jQuery( document ).ready( function() {
  bosa_clinic_slickNavHeight();
  bosa_clinic_wpMenuAccordion( '#offcanvas-menu' );
  bosa_clinic_header_two_postion();
  ...
  */
  $( '.header-image-slider' ).slick({
      ...
      fade: BOSACLINIC.header_image_slider.fade,
      speed: parseInt( BOSACLINIC.header_image_slider.fadeControl ),
      cssEase: 'linear',
      autoplay: BOSACLINIC.header_image_slider.autoplay,
      autoplaySpeed: BOSACLINIC.header_image_slider.autoplaySpeed,
      infinite: true,
  ...
  */
  $( '#primary-menu' ).slicknav({
      ...
      label: BOSACLINIC.responsive_header_menu_text,
  ...
  */
  $( '.main-slider' ).slick({
      ...
      arrows: true,
      adaptiveHeight: false,
      fade: BOSACLINIC.main_slider.fade,
      speed: parseInt( BOSACLINIC.main_slider.fadeControl ),
      cssEase: 'linear',
      autoplay: BOSACLINIC.main_slider.autoplay,
      autoplaySpeed: BOSACLINIC.main_slider.autoplaySpeed,
      infinite: true,
  ...
  $( '.highlight-post-slider' ).slick({
      arrows: true,
      dots: true,
      slidesToShow: BOSACLINIC.home_highlight_posts.slidesToShow,
      slidesToScroll: 1,
      adaptiveHeight: false,
      autoplay: BOSACLINIC.home_highlight_posts.autoplay,
      autoplaySpeed: BOSACLINIC.home_highlight_posts.autoplaySpeed,
      infinite: false,
      rows: 0,
...
jQuery( window ).on( 'load', function(){
  ...
  * Back to top
  */
  if( BOSACLINIC.enable_scroll_top == true && $(window).scrollTop() > 200 ){
...
jQuery( window ).on( 'scroll', function(){
  ...
  * Back to top
  */
  if( BOSACLINIC.disable_scroll_top == false && $(window).scrollTop() > 200 ){
    $( '#back-to-top' ).fadeIn( 200 );
  } else{
    $( '#back-to-top' ).fadeOut( 200 );
  }
