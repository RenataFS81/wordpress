...
class Magty_Posts_Carousel extends Magty_Widget_Base {
	...
	public function __construct() {
		...
		$this->widget_cssclass    = 'widget_magty_posts_carousel';
		$this->widget_description = __( 'Displays posts in carousel', 'magty' );
		$this->widget_id          = 'magty_posts_carousel';
		$this->widget_name        = __( 'Magty: Posts Carousel', 'magty' );
		$this->settings           = array(
			'title'                     => array(
				'type'  => 'text',
			...
			'margin'                    => array(
				...
				'max'   => 100,
				'std'   => 24,
				'label' => __( 'Carousel margin', 'magty' ),
			),
			'autoplay'                  => array(
				'type'  => 'checkbox',
	...
	public function get_posts( $args, $instance ) {
		...
		return new WP_Query( apply_filters( 'magty_posts_carousel_query_args', $query_args ) );
	}
	...
	public function widget( $args, $instance ) {
		...
		if ( ( $posts = $this->get_posts( $args, $instance ) ) && $posts->have_posts() ) {
			...
			do_action( 'magty_before_posts_carousel' );
			...
			if ( 'style_2' == $display_style ) {
				$overlay = '<span aria-hidden="true" class="magty-block-overlay overlay_w_gradient"></span>';
			}
			...
			if ( $dots ) {
				...
				$wrapper_class .= ' magty-swiper-outer-bullets';
			}
			...
			if ( $justify_content ) {
				$wrapper_class .= ' magty-justify-article-contents';
			}
			?>

			<div class="magty-slider-wrapper-block as_carousel <?php echo esc_attr( $wrapper_class ); ?>">
			...
			do_action( 'magty_after_posts_carousel' );
