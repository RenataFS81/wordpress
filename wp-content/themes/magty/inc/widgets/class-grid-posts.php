...
class Magty_Grid_Posts extends Magty_Widget_Base {
	/**
	 * Constructor.
	 */
	public function __construct() {

		$this->widget_cssclass    = 'magty_grid_posts_widget';
		$this->widget_description = __( 'Displays posts in grid style', 'magty' );
		$this->widget_id          = 'magty_double_column_posts';
		$this->widget_name        = __( 'Magty: Grid Posts', 'magty' );
		$this->settings           = array(
			'title'                     => array(
				'type'  => 'text',
			...
			'card_layout'               => array(
				'type'    => 'select',
				'label'   => __( 'Card Layout', 'magty' ),
				'options' => array(
					''              => __( '&mdash; Select &mdash;', 'magty' ),
					'card_layout_1' => __( 'Layout One', 'magty' ),
					'card_layout_2' => __( 'Layout Two', 'magty' ),
				),
				'std'     => '',
			),
			...
			'hide_image'                => array(
				'type'  => 'checkbox',
				'label' => __( 'Hide Image', 'magty' ),
				'std'   => false,
			),
			'post_counter_position'     => array(
				'type'    => 'select',
				'label'   => __( 'Post Counter', 'magty' ),
				'options' => array(
					''             => __( '&mdash; Select &mdash;', 'magty' ),
	...
	public function get_posts( $args, $instance ) {
		...
		return new WP_Query( apply_filters( 'magty_grid_posts_query_args', $query_args ) );
	}
	...
	public function widget( $args, $instance ) {
		...
		if ( ( $posts = $this->get_posts( $args, $instance ) ) && $posts->have_posts() ) {
			...
			do_action( 'magty_before_grid_posts' );
			...
			if ( 1 == $column ) {
				$col_class .= ' magty-grid-1';
			} elseif ( 2 == $column ) {
				$col_class .= ' row-cols-sm-2 magty-grid-2';
			} elseif ( 3 == $column ) {
				$col_class .= ' row-cols-md-3 magty-grid-3';
			} elseif ( 4 == $column ) {
				$col_class .= ' row-cols-sm-2 row-cols-xl-4 magty-grid-4';
			} elseif ( 5 == $column ) {
				$col_class .= ' row-cols-sm-2 row-cols-md-3 row-cols-lg-4 row-cols-xl-5 magty-grid-5';
			} else {
				$col_class .= ' row-cols-sm-2 magty-grid-2';
			}
			...
			if ( $justify_content ) {
				$widget_class .= ' magty-justify-article-contents';
			}
			...
			<div class="magty-grid-posts-widget <?php echo esc_attr( $widget_class ); ?>">
			...
			do_action( 'magty_after_grid_posts' );
	...
	public function enqueue_assets() {
		magty_widget_css( $this->id_base, 'grid-posts' );
	}
}
